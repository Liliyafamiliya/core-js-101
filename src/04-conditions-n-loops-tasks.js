...
function isInsideCircle(circle, point) {
  const xDistance = circle.center.x - point.x;
  const yDistance = circle.center.y - point.y;
...
function isCreditCardNumber(ccn) {
  const arrReverse = `${ccn}`.split('').reverse();
  const arrOfChars = [];
  for (let i = 0; i < arrReverse.length; i += 1) {
    if (i % 2 !== 0) {
      const doubled = parseInt(arrReverse[i], 10) * 2;
      arrOfChars.push(doubled > 9 ? doubled - 9 : doubled);
    } else {
      arrOfChars.push(parseInt(arrReverse[i], 10));
    }
  }
  const sum = arrOfChars.reduce((a, b) => a + b, 0);
  return sum % 10 === 0;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (module[char]) {
      stack.push(char);
    } else if (Object.values(module).includes(char)) {
      const topOfStack = stack.pop();
      if (module[topOfStack] !== char) {
        return false;
      }
    }
  }
...
function toNaryString(num, n) {
  ...
  let copyNum = num;
  while (copyNum > 0) {
    const theRest = copyNum % n;
    result = theRest + result;
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < firstPathParts.length; i += 1) {
    ...
    const isCommon = pathes.every((path) => path.split('/')[i] === currentPart);
    if (isCommon) {
...
function evaluateTicTacToePosition(position) {
  ...
  const resultLine = (line) => {
    const [a, b, c] = line.map((idx) => position[Math.floor(idx / 3)][idx % 3]);
    if (a === b && b === c) {
      if (a === 'X' || a === '0') {
        return a;
      }
